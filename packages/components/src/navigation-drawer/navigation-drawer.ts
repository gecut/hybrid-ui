...
export class GecutNavigationDrawerDirective extends GecutDirective {
  ...
  render(content?: NavigationDrawerContent): unknown {
    ...
    return html`
      ...
      >
        <nav
          class="flex h-full flex-col bg-surfaceContainerLow px-3 py-3 elevation-1"
        >
      ...
      (groupContent) => html`
        <div role="tablist" class="text-labelLarge text-onSurfaceVariant">
          ${when(
            groupContent.title,
            () => html`
              <h2 class="mx-4 pb-3 pt-5 text-titleSmall text-onSurfaceVariant">
                ${groupContent.title}
              </h2>
            `
          )}
  ...
  protected _renderItems(
    content: NavigationDrawerGroupContent,
    _selected: string
  ): unknown {
    return mapObject(this, content.paths, (item, name) => {
      ...
      return html`
        ...
        >
          ${icon(item.icon)}
          ${when(label, () => html`<div class="mx-2 grow">${label}</div>`)}
          ${when(
            item.badge != null,
            () => html`<div class="me-3">${item.badge}</div>`
          )}
        </a>
      `;
    });
